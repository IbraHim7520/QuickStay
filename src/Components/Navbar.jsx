...
const Navbar = () => {
  ...
  return (
    <div className="navbar sticky z-50 top-0  flex justify-between items-center px-3 md:px-8  lg:px-16 bg-base-200  shadow-sm">
      <div className="flex justify-center items-center gap-2 ">
        <div className="dropdown  block md:hidden">
          ...
          <ul
            tabIndex={0}
            className=" dropdown-content  xl:text-lg font-semibold flex flex-col space-y-3 bg-base-300 rounded-box shadow-md z-1 mt-3 w-52 p-2">
      ...
      <div className='flex justify-center items-center'>
          <img src={Logo} className='w-12 md:w-12'></img>
        <Link><h1 className='md:text-xl font-bold text-gray-700'>QuickStay</h1></Link>
      </div>
      </div>

      <div className="hidden md:block">
        <ul className='flex justify-center xl:text-lg font-semibold items-center gap-3'>
          <NavLink to={'/'} className={({ isActive }) => isActive ? "text-blue-500" : "text-black"} >Home</NavLink>
          <NavLink to={'/rooms'} className={({ isActive }) => isActive ? "text-blue-500" : "text-black"} >Rooms</NavLink>
          {
